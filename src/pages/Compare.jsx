        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-20">
          ...
          >
            ...
            <p className="body-lg text-on-surface/40 max-w-xl">
              Synchronizing telemetry across{" "}
              <span className="text-on-surface font-bold">
                {compareList.length}
              </span>{" "}
              active units. Real-time engineering dossiers compiled for
                    ...
                    >
                      ...
                      <div className="relative z-10 flex flex-col items-center">
                        ...
                        <div className="text-center space-y-1">
                          <span className="label-md text-secondary opacity-40 italic tracking-widest">
                            UNIT_0{idx + 1}
                          </span>
                      ...
                      {!isSearchActive ? (
                        <div className="flex flex-col items-center gap-6 text-center">
                          ...
                          <div className="space-y-2">
                            <p className="label-md text-on-surface/30 tracking-widest uppercase">
                              Await Selection
                            </p>
                            <Button
                              variant="ghost"
                        ...
                        <div className="w-full space-y-4">
                          <div className="relative">
                            ...
                            <input
                              ...
                              onBlur={() =>
                                !searchTerm && setIsSearchActive(false)
                              }
                            />
                          </div>
                          <div className="max-h-48 overflow-y-auto custom-scrollbar space-y-2 p-1">
                            {filteredCars.map((c) => (
                              ...
                              >
                                <img
                                  src={c.image}
                                  className="w-10 h-10 object-contain p-1 bg-black/40 rounded-lg border border-white/5"
                                  alt="icon"
                                />
                                <div className="flex flex-col">
                                  ...
                                  <span className="label-sm opacity-40 uppercase tracking-tighter">
                                    {c.brand}
                                  </span>
                                </div>
                              </button>
        ...
        {compareList.length > 0 ? (
                ...
                <tr className="border-b border-white/10 bg-white/2">
                  <th className="p-8 md:p-12 w-1/4">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="headline-sm normal-case text-on-surface">
                        Spec <span className="text-primary italic">Matrix</span>
                      </h3>
                    </div>
                  </th>
                  {compareList.map((car, idx) => (
                    <th
                      key={car.id}
                      className="p-8 md:p-12 text-center border-l border-white/5"
                    >
                      <div className="space-y-2">
                        <span className="label-sm text-secondary opacity-40 italic tracking-[0.2em] uppercase text-[10px]">
                          UNIT_0{idx + 1}
                        </span>
                        <h4 className="headline-sm normal-case text-on-surface truncate max-w-[180px] mx-auto leading-tight">
                          {car.name}
                        </h4>
                      </div>
                    </th>
                  ))}
                  {[...Array(Math.max(0, 3 - compareList.length))].map(
                    (_, i) => (
                      <th
                        key={`h-e-${i}`}
                        className="p-8 md:p-12 text-center border-l border-white/5 opacity-10"
                      >
                        <span className="label-md uppercase tracking-widest text-[10px]">
                          Awaiting Unit
                        </span>
                      </th>
                    ),
              ...
              <tbody>
                {specRows.map((row, rIdx) => {
                  ...
                  return (
                    <tr
                      key={row.key}
                      className={`group/row transition-all hover:bg-white/[0.03] ${rIdx !== specRows.length - 1 ? "border-b border-white/5" : ""}`}
                    >
                      <td className="p-8 md:p-10 px-12">
                        <div className="flex items-center gap-4 opacity-50 group-hover/row:opacity-100 transition-opacity">
                          ...
                          <span className="label-md text-on-surface text-[13px] font-bold group-hover/row:text-primary transition-colors">
                            {row.label}
                          </span>
                        </div>
                      </td>
                      {compareList.map((car) => (
                        <td
                          key={car.id}
                          className={`p-8 md:p-10 text-center border-l border-white/5 transition-all group-hover/row:bg-primary/[0.01] ${row.highlight ? "text-primary font-black display-sm text-2xl tracking-tighter" : "text-on-surface body-lg"}`}
                        >
                      ...
                      {[...Array(Math.max(0, 3 - compareList.length))].map(
                        (_, i) => (
                          <td
                            key={`d-e-${i}`}
                            className="p-8 md:p-10 text-center border-l border-white/5 opacity-5 font-mono"
                          >
                            —
                          </td>
                        ),
          ...
          >
            ...
            <div className="text-center space-y-4 max-w-sm">
              <h3 className="display-md normal-case italic text-on-surface">
                Matrix Offline
              </h3>
              <p className="body-lg text-on-surface/30">
                Initialize at least two machine units to start comparative
                signal analysis and engineering cross-references.
              </p>
            </div>
          </motion.div>
        )}
        ...
        >
          ...
          <div className="relative z-10 space-y-4 text-center lg:text-left">
            ...
            <h4 className="display-md normal-case italic text-on-surface">
              Generate{" "}
              <span className="text-secondary not-italic">Dossier</span>
            </h4>
            <p className="body-lg text-on-surface/40 max-w-md">
              Export complete performance specifications and engineering logs
              for all selected units in a encrypted PDF format.
            </p>
          </div>
