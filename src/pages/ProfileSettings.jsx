...
import { 
  ...
  Activity, Cpu, Fingerprint, Database
} from 'lucide-react';
...
const ProfileSettings = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    ...
    defaultValues: {
      ...
      manifesto: 'High-performance enthusiast. Tracking telemetry for the Spectre GT and Nebula EV fleet. Focused on precision engineering and efficiency.',
    }
  });
    ...
    <div className="min-h-screen bg-surface selection:bg-primary/30 pt-32 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          ...
          >
            <h1 className="display-md normal-case leading-none mb-4 text-on-surface">Profile <span className="text-primary italic">Control</span></h1>
          ...
          <aside className="lg:col-span-3 space-y-6">
            <div className="glass-card p-6 rounded-[2.5rem] overflow-hidden group relative">
              ...
              <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/5 relative z-10">
                ...
                <div>
                  <div className="headline-md normal-case text-on-surface text-base">JUL_THORNE</div>
                  <div className="label-md text-primary opacity-60">Apex Tier</div>
                </div>
              </div>
                ...
                {tabs.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isActive && <motion.div layoutId="active-pill" className="w-1 h-1 rounded-full bg-primary" />}
                    </button>
                  );
                })}
              </nav>
              ...
              <button className="flex items-center gap-4 px-6 py-5 rounded-2xl text-on-error/80 hover:bg-error/10 hover:text-error transition-all mt-6 border border-white/5 hover:border-error/20 w-full">
                <LogOut size={18} />
                <span className="body-md font-bold uppercase tracking-widest text-[10px]">Terminate Session</span>
              </button>
            </div>
            ...
            <div className="glass-card p-6 rounded-[2rem] flex items-center gap-5 group">
              ...
              <div>
                ...
                <div className="headline-md normal-case text-on-surface text-lg">Stable v2.4</div>
              </div>
            </div>
          </aside>
              ...
              >
                ...
                {activeTab === 'profile' && (
                  <div className="relative z-10 space-y-12">
                    <div className="flex items-center gap-4 border-b border-white/5 pb-8">
                      ...
                      <div>
                        <h3 className="headline-md normal-case text-on-surface">Pilot Profile</h3>
                        <p className="body-md opacity-40">Your operational identifiers and mission settings.</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
                      <div className="xl:col-span-4 flex flex-col items-center gap-6">
                        ...
                        <span className="label-md opacity-30 mt-4 tracking-[0.3em]">REWRITE_BIOMETRICS</span>
                      </div>

                      <div className="xl:col-span-8 space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="space-y-3">
                            <label className="body-md font-bold text-on-surface/40 ml-2">Operational Callsign</label>
                            ...
                            {errors.callsign && <p className="text-error text-[10px] uppercase font-bold ml-2">{errors.callsign.message}</p>}
                          </div>
                          
                          <div className="space-y-3">
                            <label className="body-md font-bold text-on-surface/40 ml-2">Secure Channel (Email)</label>
                            <div className="relative group/input">
                            ...
                            {errors.email && <p className="text-error text-[10px] uppercase font-bold ml-2">{errors.email.message}</p>}
                          </div>
                        </div>

                        <div className="space-y-3">
                          <label className="body-md font-bold text-on-surface/40 ml-2">Signal Tel</label>
                          <div className="relative group/input">
                          ...
                          {errors.phone && <p className="text-error text-[10px] uppercase font-bold ml-2">{errors.phone.message}</p>}
                        </div>

                        <div className="space-y-3">
                          <label className="body-md font-bold text-on-surface/40 ml-2">Pilot Manifesto (Bio)</label>
                          <textarea 
                            {...register('manifesto')}
                          ...
                          />
                          {errors.manifesto && <p className="text-error text-[10px] uppercase font-bold ml-2">{errors.manifesto.message}</p>}
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'security' && (
                  <div className="relative z-10 space-y-12">
                    <div className="flex items-center gap-4 border-b border-white/5 pb-8">
                      ...
                      <div>
                        <h3 className="headline-md normal-case text-on-surface">Security Protocols</h3>
                        <p className="body-md opacity-40">Encryption nodes and biometric authentication settings.</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={idx} className="flex items-center justify-between p-8 rounded-3xl bg-white/5 border border-white/5 group hover:border-white/10 transition-all">
                          <div className="flex items-center gap-6">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${prot.enabled ? 'bg-primary/10 text-primary' : 'bg-white/5 text-on-surface/20'}`}>
                              <prot.icon size={26} />
                            </div>
                            <div>
                              <h4 className="headline-md normal-case text-base text-on-surface mb-1">{prot.title}</h4>
                              <p className="body-md text-on-surface/40 max-w-sm">{prot.desc}</p>
                            </div>
                          </div>
                          
                          <div className={`w-14 h-8 rounded-full p-1.5 transition-colors duration-500 cursor-pointer ${prot.enabled ? 'bg-primary' : 'bg-surface-highest'}`}>
                            <motion.div 
                    ...
                    <div className="pt-8 flex justify-center">
                      <Button variant="outline" className="h-16 px-12 rounded-2xl border-white/10 text-on-surface group">
                        <Lock size={18} className="mr-3 group-hover:text-primary transition-colors" />
                ...
                {activeTab === 'garage' && (
                  <div className="relative z-10 text-center py-24">
                    <Activity size={64} className="mx-auto mb-6 text-primary opacity-20" />
                    <h3 className="headline-md normal-case text-on-surface mb-2">Fleet Management</h3>
                    <p className="body-lg opacity-40">Section under active deployment cycle.</p>
                    <Button variant="outline" className="mt-8">Request Early Access</Button>
                  </div>
                )}
              </motion.div>
            </AnimatePresence>
