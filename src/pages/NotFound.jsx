...
import { Home, Search, AlertCircle, RefreshCcw, WifiOff, Map, Compass, Terminal } from 'lucide-react';
      ...
      <main className="relative z-10 w-full max-w-5xl px-6 py-24 flex flex-col items-center text-center">
        ...
        >
          ...
          <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center opacity-10 -z-10 group">
             <span className="text-[15rem] font-black italic tracking-tighter opacity-10 group-hover:opacity-20 transition-opacity">VOID</span>
          </div>
        </motion.div>
        ...
        >
          <div className="space-y-4">
             <div className="flex items-center justify-center gap-4 mb-2">
                ...
                <span className="label-sm text-secondary uppercase tracking-[0.5em] font-black">Transmission Interrupted</span>
                <div className="w-10 h-[1px] bg-secondary/40"></div>
             </div>
             ...
             <p className="body-lg text-on-surface/40 max-w-xl mx-auto leading-relaxed">
               The coordinates you requested do not exist in the Kinetic database. The link has been severed or the destination page has been de-synchronized.
             </p>
          </div>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-8 pt-12">
            <Link to="/">
               <Button size="lg" className="px-16 h-20 rounded-[2.5rem] shadow-luxury-neon flex items-center gap-4 group/btn active:scale-95">
                  <Home size={20} className="group-hover:-translate-y-1 group-hover:text-black transition-all" />
                  <span className="label-md font-black uppercase tracking-[0.3em]">Return to Command Station</span>
               </Button>
            </Link>
            <Link to="/cars">
               <Button size="lg" variant="ghost" className="px-16 h-20 rounded-[2.5rem] border-white/5 hover:border-primary/40 glass-card flex items-center gap-4 group/btn active:scale-95">
                  <Search size={20} className="group-hover:scale-110 transition-transform" />
                  <span className="label-md font-black uppercase tracking-[0.3em]">Search Grid Data</span>
               </Button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl pt-20 border-t border-white/5">
             <div className="flex flex-col items-center gap-3 group opacity-40 hover:opacity-100 transition-all">
                ...
                <span className="label-sm uppercase tracking-widest font-black">Signal Lost</span>
             </div>
             <div className="flex flex-col items-center gap-3 group opacity-40 hover:opacity-100 transition-all">
                <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:text-primary transition-colors">
                   <Compass size={20} />
                </div>
                <span className="label-sm uppercase tracking-widest font-black">Bearing Invalid</span>
             </div>
             <div className="flex flex-col items-center gap-3 group opacity-40 hover:opacity-100 transition-all">
                <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:text-primary transition-colors">
                   <RefreshCcw size={20} />
                </div>
                <span className="label-sm uppercase tracking-widest font-black">Recalibrating...</span>
             </div>
          </div>
        </motion.div>
        ...
        <div className="absolute bottom-12 left-12 hidden lg:block text-left space-y-2">
           ...
           <p className="text-[10px] text-on-surface/20 font-mono tracking-widest uppercase">
              ERR_CODE: LINK_STATION_TIMEOUT<br/>
              TIMESTAMP: 30.MAR.26_17:06:51<br/>
