        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-20">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
               ...
               <span className="label-md text-primary opacity-60">Private Hangar</span>
            </div>
            ...
            <p className="body-lg text-on-surface/40 max-w-xl">
              Your personal grid of peak engineering. Monitoring <span className="text-on-surface font-bold">{wishlistedCars.length}</span> active units in your local synchronization.
            </p>
          </motion.div>
          ...
          >
            ...
            <Link to="/listing">
              <Button size="lg" className="px-10 h-14 rounded-2xl flex items-center gap-2 group">
                <Sparkles size={18} className="group-hover:rotate-45 transition-transform" />
                Initialize Scan
              </Button>
            </Link>
          </motion.div>
          ...
          {wishlistedCars.length > 0 ? (
                ...
                >
                  ...
                  <div className="relative h-64 overflow-hidden bg-white/5 flex items-center justify-center p-8 group">
                    ...
                    <div className="absolute bottom-6 left-8">
                       <span className="label-md bg-white/5 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/5 opacity-40">Unit_{car.id.slice(-4)}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-10 space-y-8">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="headline-md normal-case text-on-surface mb-1 group-hover:text-primary transition-colors">{car.name}</h3>
                        <p className="body-md opacity-40">{car.brand} • {car.category}</p>
                      </div>
                      <div className="text-right">
                        <span className="display-md text-2xl font-bold tracking-tight text-primary">${car.price.toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="bg-white/5 p-4 rounded-2xl border border-white/5 group-hover:border-primary/10 transition-colors">
                          <div className="flex items-center gap-2 mb-1.5 opacity-30">
                             ...
                             <span className="label-md text-[8px]">Propulsion</span>
                          </div>
                          <span className="body-md font-bold text-on-surface truncate block">{car.engine}</span>
                       </div>
                       <div className="bg-white/5 p-4 rounded-2xl border border-white/5 group-hover:border-secondary/10 transition-colors">
                          ...
                          <span className="body-md font-bold text-on-surface">{car.acceleration}</span>
                       </div>
                    </div>
                    ...
                    >
                      Retrieve Data <ArrowRight className="ml-2 w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                    </Button>
                  </div>
                </motion.div>
              ))}
            ...
            >
              ...
              <div className="space-y-4 text-center max-w-sm">
                <h3 className="display-md normal-case text-on-surface italic">Grid Empty</h3>
          ...
          >
            ...
            <div className="relative z-10 space-y-4 text-center lg:text-left">
               <div className="flex items-center justify-center lg:justify-start gap-4">
                  ...
                  <span className="label-md text-primary opacity-60">Enhanced Intelligence</span>
               </div>
               <h4 className="display-md normal-case italic text-on-surface">Want more <span className="text-secondary not-italic">Telemetry?</span></h4>
               <p className="body-lg text-on-surface/40 max-w-md">
                  Connect your uplink to our neural network for real-time inventory updates and peak performance dossiers.
               </p>
            </div>
            ...
            >
               Synchronize Grid <Sparkles className="ml-3 w-4 h-4 group-hover/cta:rotate-45 transition-transform" />
            </Button>
