        ...
        <div className="bg-background text-white font-inter overflow-hidden">
            ...
            <section className="relative h-[85vh] flex items-center overflow-hidden">
                ...
                <div className="relative z-10 px-8 md:px-20 max-w-5xl">
                    ...
                    >
                        <span className="label-md text-primary tracking-[0.5em]">SINCE 2024</span>
                    </motion.div>
                    <motion.h1 
            ...
            <section className="py-40 px-8 bg-surface-container-low border-y border-white/5">
                <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
                    ...
                    >
                        ...
                        <h2 className="display-md mb-10">THE PRECISION <br/><span className="text-primary italic">NARRATIVE.</span></h2>
                        ...
                        <div className="grid grid-cols-2 gap-12 pt-12 border-t border-white/5 mt-12">
                            {stats.slice(0, 2).map((stat, idx) => (
                                <div key={idx}>
                                    <span className="block text-5xl font-poppins font-bold text-primary tracking-tighter mb-2 italic leading-none">{stat.value}</span>
                                    <span className="text-[0.65rem] uppercase tracking-[0.3em] font-poppins font-bold text-on-surface-variant opacity-40">{stat.label}</span>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative">
                        ...
                        <div className="absolute -bottom-10 -left-10 glass-card p-12 hidden md:block max-w-sm">
                            <p className="font-poppins font-bold text-2xl text-primary leading-tight lowercase italic">"The difference between a machine and a soul is the sound it makes at 9,000 RPM."</p>
                        </div>
                    </motion.div>
                </div>
            </section>
            ...
            <section className="py-48 px-8">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-24">
                        <span className="text-primary font-poppins font-bold tracking-[0.5em] uppercase text-[0.65rem] mb-6 block opacity-60">PHILOSOPHICAL DNA</span>
                        <h2 className="text-6xl md:text-7xl font-poppins font-bold uppercase tracking-tighter leading-none">CORE <span className="text-primary italic">PRINCIPLES.</span></h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        {principles.map((p, idx) => (
                            <motion.div 
                                key={idx}
                                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1, duration: 0.8 }}
                            ...
                            >
                                <div className="mb-12 w-20 h-20 rounded-2xl bg-primary/5 flex items-center justify-center group-hover:scale-110 group-hover:bg-primary/10 transition-all duration-500">
                                    {React.cloneElement(p.icon, { size: 32, className: "text-primary" })}
                                </div>
                                <h3 className="text-3xl font-poppins font-bold mb-6 uppercase tracking-tight italic">{p.title}</h3>
                                <p className="text-on-surface-variant font-medium text-lg leading-relaxed opacity-60 group-hover:opacity-90 transition-opacity">{p.desc}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-24 gap-12">
                        <div className="space-y-4">
                            <span className="text-on-surface-variant font-poppins font-bold tracking-[0.4em] uppercase text-[0.65rem] opacity-40">THE VISIONARIES</span>
                            <h2 className="text-6xl font-poppins font-bold uppercase tracking-tighter leading-none">ARCHITECTS <br/><span className="text-white/30">OF SPEED.</span></h2>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                        {leaders.map((leader, idx) => (
                            <motion.div 
                                key={idx}
                                initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: idx * 0.1, duration: 0.8 }}
                                className="flex flex-col items-center group text-center"
                            >
                                <div className={`w-full aspect-[4/5] rounded-[2rem] overflow-hidden mb-10 border border-white/10 group-hover:border-primary/30 transition-all duration-500 relative shadow-2xl`}>
                                    <img src={leader.img} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100" alt={leader.name} />
                                ...
                                <h4 className="text-2xl font-poppins font-bold uppercase tracking-tight mb-2">{leader.name}</h4>
                                <span className="text-[0.6rem] uppercase tracking-[0.4em] font-poppins font-bold text-primary opacity-60">{leader.role}</span>
                            </motion.div>
                        ))}
                    </div>
                </div>
