...
import { Search, ArrowLeft, Filter, SlidersHorizontal, AlertCircle, Sparkles, Terminal, Activity, Zap, Home } from 'lucide-react';
      ...
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-12">
            <div className="space-y-6">
               <Link to="/listing" className="group inline-flex items-center gap-4 label-sm font-black tracking-[0.4em] text-on-surface/40 hover:text-primary transition-all uppercase italic">
               ...
               <div className="space-y-4">
                  ...
                  >
                     Search <span className="text-secondary not-italic">Analytics</span>
                  </motion.h1>
                  <div className="flex flex-wrap items-center gap-4">
                     <div className="glass-card px-8 py-4 rounded-2xl border border-white/10 flex items-center gap-4 group hover:border-primary/40 transition-all">
                        ...
                        <span className="headline-xs normal-case italic text-on-surface">"{query.toUpperCase()}"</span>
                     </div>
                     <div className="glass-card px-8 py-4 rounded-2xl border border-white/10 flex items-center gap-4">
                        ...
                        <span className="label-md text-on-surface font-black tracking-widest uppercase">{filteredCars.length} MATCHES DETECTED</span>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="hidden xl:flex items-center gap-6 p-8 glass-card rounded-[2.5rem] border border-white/5 max-w-sm">
               ...
               <div className="space-y-1">
                  <p className="label-sm text-on-surface font-black uppercase tracking-widest">Neural Link Active</p>
                  <p className="text-[10px] text-on-surface/20 uppercase tracking-[0.2em] font-bold">Scanning Global Grid database...</p>
               </div>
            </div>
        </div>
             ...
             {filteredCars.length > 0 ? (
               ...
               >
                 ...
                 <div className="space-y-4 relative z-10">
                   <h2 className="display-sm normal-case italic text-on-surface">Zero <span className="text-secondary not-italic">Matches</span></h2>
                   <p className="label-md text-on-surface/30 font-black uppercase tracking-[0.3em] max-w-md mx-auto leading-relaxed">
                     No telemetry signals identified for query: <span className="text-secondary italic">"{query}"</span>. 
                     Recalibrate your search parameters or explore the full fleet inventory.
                   </p>
                 </div>

                 <div className="flex flex-col sm:flex-row gap-6 relative z-10">
                   <Link to="/listing">
                     <Button size="lg" className="px-12 h-18 rounded-3xl shadow-luxury-neon flex items-center gap-4">
                       Explore Elite Fleet <Sparkles size={18} />
                     </Button>
                   </Link>
                   <Link to="/">
                     <Button size="lg" variant="ghost" className="px-12 h-18 rounded-3xl border-white/5 hover:border-white/20 glass-card flex items-center gap-4">
        ...
        {filteredCars.length < 3 && filteredCars.length > 0 && (
           ...
           >
             <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                <div className="space-y-4">
                   <span className="label-sm text-primary uppercase tracking-[0.5em] font-black italic opacity-60">Strategic Recommendations</span>
                   <h3 className="display-xs normal-case italic text-on-surface leading-none">
                      Alternative <span className="text-secondary not-italic">Protocols</span>
                   </h3>
                </div>
                <Link to="/listing" className="label-sm font-black uppercase tracking-[0.3em] text-on-surface/40 hover:text-primary transition-all flex items-center gap-3">
