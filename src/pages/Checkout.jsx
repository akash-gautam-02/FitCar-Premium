        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          >
            <header className="flex items-center gap-6 mb-16">
              ...
              <h1 className="display-md normal-case italic text-on-surface">
                Secure{" "}
                <span className="text-primary not-italic">Settlement</span>
              </h1>
            </header>
            ...
            <div className="space-y-16">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  {[
                    ...
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() =>
                        setValue("method", method.id, { shouldValidate: true })
                      }
                    ...
                    >
                      ...
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${paymentMethod === method.id ? "bg-primary text-black" : "bg-white/5 text-on-surface/30 group-hover:text-primary/60"}`}
                      >
                        <method.icon size={28} />
                      </div>
                      <div className="space-y-1 relative z-10">
                        <span
                          className={`label-md uppercase font-black tracking-[0.2em] block ${paymentMethod === method.id ? "text-primary" : "text-on-surface/40"}`}
                        >
                          {method.name}
                        </span>
                        <span className="label-sm opacity-30 lowercase italic">
                          {method.desc}
                        </span>
                      </div>
                    </motion.div>
                ...
                {errors.method && (
                  <p className="label-sm text-error/80 mt-4 ml-2">
              ...
              <form
                ...
                className="glass-card p-10 md:p-12 rounded-[3.5rem] border border-white/5 space-y-10 relative overflow-hidden"
              >
                  ...
                  {paymentMethod === "card" && (
                    ...
                    >
                      <div className="space-y-3">
                        <label className="label-md uppercase font-bold text-on-surface/40 tracking-widest ml-2 italic">
                          Cardholder Identity
                        </label>
                        <input
                          {...register("cardName")}
                        ...
                        />
                        {errors.cardName && (
                          <p className="label-sm text-error/80 ml-2">
                  ...
                  {paymentMethod === "crypto" && (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <h4 className="label-md uppercase font-black text-primary tracking-widest">
                          Connect Matrix Wallet
                        </h4>
                        <p className="body-md text-primary/60 max-w-sm">
                          Authorization will be requested via your browser
                          extension after confirmation.
                        </p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {paymentMethod === "bank" && (
                    ...
                    >
                      <div className="flex items-center gap-4 text-on-surface/60">
                        <Landmark size={20} />
                        <span className="label-md uppercase font-bold tracking-widest">
                          Institution Protocol
                        </span>
                      </div>
                      <p className="body-md text-on-surface/40 leading-relaxed italic">
                        Direct ledger transfer instructions will be generated
                        and dispatched to your primary uplink address upon
                        execution. Finalization may take up to 24 atomic cycles.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="pt-4 space-y-8">
                  <div className="flex items-center gap-6 p-6 bg-white/2 border border-white/5 rounded-3xl group hover:border-primary/20 transition-colors">
                    ...
                    <div>
                      <p className="label-md uppercase font-black text-on-surface tracking-[0.2em]">
                        Quantum Encryption Active
                      </p>
                      <p className="label-sm text-on-surface/40">
                        Full node desensitization for total transaction
                        anonymity.
                      </p>
                    </div>
                  </div>
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-12">
                  <div className="flex items-center gap-3 opacity-40">
                    <Package size={18} />
                    <h3 className="label-md uppercase font-black tracking-[0.2em] text-on-surface">
                      Unit_Dossier
                    </h3>
                  </div>
                  <span className="label-sm bg-primary/10 text-primary px-4 py-1.5 rounded-full font-black italic">
                    Batch 01 // Unit_4492
                  </span>
                </div>
                ...
                <div className="flex items-center gap-8 mb-12 pb-12 border-b border-white/5">
                  <div className="w-32 h-32 bg-black/40 rounded-[2.5rem] border border-white/5 p-6 flex items-center justify-center shrink-0 relative overflow-hidden group/img shadow-2xl">
                    ...
                    <img
                      ...
                      className="w-full h-full object-contain filter grayscale group-hover/img:grayscale-0 group-hover:scale-110 transition-all duration-700 drop-shadow-luxury"
                      alt="Car Preview"
                    />
                  </div>
                  <div className="space-y-2">
                    <h4 className="display-sm text-3xl normal-case italic text-on-surface leading-none tracking-tighter">
                      KINETIC{" "}
                      <span className="text-primary not-italic">OBSIDIAN</span>
                    </h4>
                    <div className="flex flex-wrap gap-2 pt-2">
                      <span className="label-xs bg-white/5 border border-white/10 px-2 py-0.5 rounded text-on-surface/40">
                        V12 HYBRID
                      </span>
                      <span className="label-xs bg-white/5 border border-white/10 px-2 py-0.5 rounded text-on-surface/40">
                        AERO_X
                ...
                <div className="space-y-6 mb-12">
                  <div className="flex justify-between items-center group/row">
                    <span className="label-md uppercase tracking-widest text-on-surface/40 group-hover/row:text-on-surface/80 transition-colors">
                      Core Allocation
                    </span>
                    <span className="body-lg font-black text-on-surface tracking-tight">
                      ${orderSummary.basePrice.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-secondary group/row">
                    <div className="flex items-center gap-2">
                      <Sparkles size={14} className="animate-pulse" />
                      <span className="label-md uppercase tracking-widest font-black">
                        Performance Matrix
                      </span>
                    </div>
                    <span className="body-lg font-black tracking-tight">
                      + ${orderSummary.performancePack.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center group/row">
                    <span className="label-md uppercase tracking-widest text-on-surface/40 group-hover/row:text-on-surface/80 transition-colors">
                  ...
                  <div className="flex justify-between items-center group/row">
                    ...
                    <span className="body-lg font-black text-on-surface tracking-tight">
                      ${orderSummary.destinationFee.toLocaleString()}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="flex justify-between items-end">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-primary">
                        <Calculator size={14} />
                        <span className="label-sm font-black uppercase tracking-[0.4em] italic">
                          Total Settlement
                        </span>
                      </div>
                      <h4 className="display-md font-black text-5xl text-on-surface tracking-[ -0.05em] italic leading-none">
                        ${orderSummary.total.toLocaleString()}
                      </h4>
                    </div>
                    <div className="text-right">
                      <span className="label-xs text-on-surface/20 uppercase tracking-[0.3em] block mb-1">
                        VAT INCL.
                      </span>
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/5 rounded-lg border border-white/5">
                        <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                        <span className="label-xs text-primary font-black">
                          LOCKED
                        </span>
                      </div>
                    </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <CheckCircle2
                      size={18}
                      className="text-primary group-hover/status:scale-110 transition-transform"
                    />
                    <span className="label-sm text-on-surface/40 uppercase tracking-[0.2em] font-bold">
                      Allocation Secure
                    </span>
                  </div>
                  <Info
                    size={14}
                    className="text-on-surface/10 hover:text-primary transition-colors cursor-help"
                  />
                </motion.div>
              </div>
            </div>
