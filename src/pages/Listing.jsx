...
const Listing = () => {
  ...
  const fuelTypes = useMemo(
    () => [...new Set(carsData.map((car) => car.fuelType))],
    [],
  );
  ...
  const filteredCars = useMemo(() => {
    return carsData.filter((car) => {
      ...
      const matchesTrans =
        ...
        car.transmission
          .toLowerCase()
          .includes(selectedTransmission.toLowerCase());
      const matchesFuel =
    ...
    <div className="bg-background pt-24 pb-32 px-6 lg:px-20 min-h-screen relative overflow-hidden">
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="relative group">
              ...
              />
              <Search
                className="absolute right-6 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors duration-500"
                size={18}
              />
            </div>
            ...
            <div className="hidden md:flex glass-effect px-8 py-5 rounded-2xl border border-white/5 items-center gap-4">
              <span className="text-primary font-poppins font-bold text-lg italic leading-none">
                {filteredCars.length}
              </span>
              <span className="text-on-surface-variant text-[0.6rem] uppercase font-poppins font-bold tracking-[0.2em] leading-none opacity-50">
                Units Detected
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          {/* Desktop Sidebar */}
          <aside
            className={`w-full lg:w-80 shrink-0 space-y-12 ${isSidebarOpen ? "block" : "hidden lg:block"}`}
          >
            {/* Price Filter */}
            <section className="space-y-6">
              <h3 className="label-md text-[0.65rem] text-on-surface-variant flex justify-between items-center glass-effect p-5 rounded-xl border border-white/5">
                ...
                <span className="text-primary italic tracking-tight">
                  ${(priceRange[1] / 1000).toFixed(0)}K MAX
                </span>
              </h3>
              <div className="px-2">
                <input
            ...
            {/* Brand Filter */}
            <section className="space-y-6">
              <h3 className="label-md text-[0.65rem] text-on-surface-variant px-2">
                Strategic Brands
              </h3>
          ...
          <div className="flex-1">
            <AnimatePresence mode="popLayout">
              {filteredCars.length > 0 ? (
                ...
                >
                  <SlidersHorizontal
                    className="mx-auto text-primary/20 mb-8"
                    size={64}
                  />
                  <h2 className="text-4xl font-poppins font-bold uppercase text-white tracking-tighter mb-4">
                    Zero Matches
                  </h2>
                  <p className="text-on-surface-variant uppercase tracking-[0.2em] text-[0.65rem] font-bold opacity-50">
                    ADJUST TELEMETRY PARAMETERS FOR BETTER RESULTS.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
